...
class TFGraph(pygraph.DirectedGraph):
    ...
    def new_tf_edge(self, from_robot_id, to_robot_id, Rigidtrans):
        ...
        if from_robot_id < to_robot_id:
            ...
            tmp1=1
            tmp2=1.1
        edge_id = self.new_edge(from_node_id, to_node_id, tmp1)
        self.edges[edge_id]['data']['tf'] = Rigidtrans
        edge_id = self.new_edge(to_node_id, from_node_id, tmp2)
    ...
    def get_tf_tree(self, robot_id):
        ...
        # subgraph.nodes = dict(sorted(subgraph.nodes.items(),key=lambda x:x[1]['data']['robot_id']))
