...
from geometry_msgs.msg import Pose, PoseStamped, Transform, TransformStamped, Point, Vector3
from nav_msgs.msg import Path
from visualization_msgs.msg import Marker,MarkerArray
from gmm_map_python.msg import gmm, Submap, gmmFrame
    ...
    def Posemap2g2oVertex(self, g2ofname, pose_array, add=False):
        ...
        with open(g2ofname, openmethod) as g2ofile:
            for VERTEXpose in pose_array:
                ...
                print("VERTEX_SE3:QUAT {pointID} {tx} {ty} {tz} {rx} {ry} {rz} {rw}".format(pointID = vertexID,
                    tx = pose_global.position.x, ty = pose_global.position.y, tz = pose_global.position.z, 
                    rx = pose_global.orientation.x, ry = pose_global.orientation.y, rz = pose_global.orientation.z, rw = pose_global.orientation.w),file=g2ofile)
...
class InsubmapProcess:
    def __init__(self, submap_index, robot_id, init_pose_odom, init_pose_map, 
        ...
        self.clf = GaussianMixture(n_components=80, covariance_type='diag',max_iter=5) #GMM模型生成器
        self.clf_feature = GaussianMixture(n_components=80, covariance_type='diag') #GMM模型生成器
        self.GMMmodel = None
        self.Octomap = None
    ...
    def insert_gmm(self, in_point_cloud, ifcheck = True, if_filter = True): #此函数永远在insert_point中调用
        ...
        # print(in_point_cloud)
        # print(self.clf._estimate_log_prob_resp(in_point_cloud))
...
class TrajMapBuilder:
    def __init__(self, self_id):
        ...
        self.new_self_submap = True # 如果有是新的关键帧,这个会变成True,在callback_self_pointcloud中接收到新的关键帧率.并把他改成False
        ...
        self.Descriptor = Descriptor(model_dir='/home/emlia/gmm_map_ws/model13.ckpt')
        self.descriptor_first = None

        self.aaa =0
        self.other_sub_num = 0
        self.match_state = 0
        ...
        self.GMM_pub = rospy.Publisher('GMMglobal', gmm, queue_size=10)
        self.GMMvisual_pub = rospy.Publisher('GMMvisual', MarkerArray, queue_size=10)

        self.sub_subgmm = rospy.Subscriber('GMMglobal',gmm, self.GMMvisual_cb, queue_size=10)
        ...
        self.fitness_thr =0.2 #0.04 #0.03
    ...
    def submap_registration(self,inputsubmap,tosubmap): #输入两个子地图,输出两个地图的变换
        ...
        # print ("T_to_input.transform.rotation",T_to_input.transform.rotation)
        # print ("T_to_input.transform.translation",T_to_input.transform.translation)
    ...
    def callback_submap_listener(self, data): #主要是为了处理回环,包括自己的回环和别人的回环  
        ...
        # print("lalalalalalalalalalalalalalal")
        # print("submap_pose_odom",recvsubmap.submap_pose_odom)
        # print("submap_pose",recvsubmap.submap_pose)
        # print("position",recvsubmap.submap_gmm.pose.position)
        # print("orientation",recvsubmap.submap_gmm.pose.orientation)
        # print(data.pose.position) #机器人的position
        # print(recvsubmap.clf.score_samples) #可用于得到某个点的score
        # print(recvsubmap.robot_id)
        # print(recvsubmap.submap_index)
        # print(recvsubmap)
        current_robot_id="robot{}".format(recvsubmap.robot_id)
        if not current_robot_id in self.all_submap_lists: #如果听到的是全新的机器人,就新增一个
        ...
        for robotname in match.keys():
            ...
            # print("current_robot_id",current_robot_id)
            if robotname == current_robot_id: #寻找自己的回环
                ...
                match_valid = match[robotname] < self.match_thr
                # print("match_valid",match_valid)
                match_index = np.where(match_valid)
                # print("match_index",match_index)
                # print("match_index[0]",match_index[0])
                # print('match_index[0]:' + str(match_index[0]))
                ...
                for valid_index in match_index[0]: #因为只有一个维度
                    ...
                    if fitness < self.fitness_thr:
                        ...
                        self.match_state = 1 #匹配状态置1
                        state = np.array([self.match_state])
                        state = Float32MultiArray(data = state)
    ...
    def callback_self_pointcloud(self, data): #监听pointcloud,自己新建地图,并且保存对应的 odom.
        ...
        # print("data.row_step",data.row_step)
        ...
        ####判断匹配状态是否为1，如为1，则开始发布自己的位置信息。
        # try:
        #     self.tf_listener.waitForTransform(self.map_frame, self.baselink_frame, pointtime, rospy.Duration(3.0)) 
        #     (self.position,self.orientation) = self.tf_listener.lookupTransform(self.map_frame, self.baselink_frame,rospy.Time(0)) #实时更新位置和姿态
        #     self.position = np.array([self.position[0],self.position[1]])
        #     if self.match_state == 1:
        #         current_pos = np.array([self.position[0],self.position[1],self.self_robot_id])
        ...
        #     print ('no tf')
        print("check1")
        print(self.tf2_buffer.can_transform(self.baselink_frame,pointheader.frame_id, pointtime))
        # self.tf_listener.waitForTransform(self.baselink_frame,pointheader.frame_id,pointtime,rospy.Duration(0.11))
        self.transform_base_camera = self.tf2_buffer.lookup_transform(self.baselink_frame,pointheader.frame_id, pointtime)
        baselink_pointcloud = do_transform_cloud(data,self.transform_base_camera) #将 source(camera) 的点云 变换到 taget(base_link) 坐标系

        # try:
        #     self.tf_listener.waitForTransform(self.odom_frame,self.baselink_frame,pointtime,rospy.Duration(5.0))
        ...
        #     return
        print("check2")
        print(self.tf2_buffer.can_transform(self.odom_frame,self.baselink_frame, pointtime))
        # self.tf_listener.waitForTransform(self.odom_frame,self.baselink_frame,pointtime,rospy.Duration(5.0))
        transform_odom_base = self.tf2_buffer.lookup_transform(self.odom_frame,self.baselink_frame, pointtime, rospy.Duration(5.0))
        ...
        if self.new_self_submap: #说明要增加一个新的关键帧
            ...
            if not (self.prefixsubmap_builder == None): #如果不是第一帧,就需要把之前的帧给保存下来    
                ...
                #用点云构建GMMsubmap
                # if self.prefixsubmap_builder.submap_index%2 ==0:
    ...
    def pointmap_merge_single(self): #将每一帧的点云都合并在一起可视化出来
        # self.aaa = self.prefixsubmap_builder.submap_index 
        # if self.other_sub_num != 0:
        #     self.other_sub_num = self.aaa
        # print("self.aaa",self.aaa)
        # print("self.other_sub_num",self.other_sub_num)
        # # final_result = PointCloud2()
        # tmpmap_builder = InsubmapProcess(0,0,Pose(),Pose(),self.Descriptor,add_time=None, point_clouds=np.zeros((0,3)), init_gmm = self.submap_before, freezed=False, descriptor=None, feature_point=np.zeros((0,3)), feature_gmm=GMMFrame())
        ...
        #         # tf_inter_robot = self.tf_graph.get_tf(int(robot_id[5:]), self.self_robot_id)
        #         # if tf_inter_robot == None:
        #         if self.match_state == 0: #如果没有匹配成功，则不共享地图
        #             continue
        ...
        #         if robot_id != 'robot{}'.format(self.self_robot_id) and self.other_sub_num != self.aaa:
        #             submapinst_tmp=submapinst.gmmmap2odom(tf_inter_robot)#GMM坐标系转换，因为gmmmap2odom会改自身的参数，所以要新开一个变量暂存
        #             tmpmap_builder.GMMmerge_global(submapinst_tmp)    
        #             self.submap_before = tmpmap_builder.submap_gmm #GMM参数添加
        #             self.other_sub_num += 1
        #             print("huanyuanlingyigededitu~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~`")

        #         if submapinst.submap_index == self.aaa:
        #             submapinst_tmp=submapinst.gmmmap2odom(tf_inter_robot)#GMM坐标系转换，因为gmmmap2odom会改自身的参数，所以要新开一个变量暂存
        ...
        odom_pose=Pose()
        # print("submap_pose_odom",tmpmap_builder.submap_pose_odom)
        # print("submap_pose",tmpmap_builder.submap_pose)
        ...
        if showGMMs.mix_num!=0 :
            ...
            # rospy.loginfo(showGMMs.mix_num)

        # data = Float32MultiArray(data=a)
        # self.GMM_pub1.publish(data)
        # rospy.loginfo(data)
        # self.GMM_pub1.publish(a)
        # rospy.loginfo(b)
