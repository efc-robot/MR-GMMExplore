...
class Navigation:
    def __init__(self, self_id):
        ...
        self.sub_subgmm = rospy.Subscriber('GMMglobal',gmm, self.callback_nav,queue_size=10)  # spacial info extractor + goal selector
        self.sub_subgmmtrans = rospy.Subscriber('/all_submap', Submap, self.callback, queue_size=10)
        self.sub_target = rospy.Subscriber('target_pos',Float32MultiArray,self.gmm_nav,queue_size=10) # local planner
    ...
    def obstacle_free(self, x0, y0, x1, y1, x2, y2):
        ...
        """
        #取机器人的位置：x0, y0 = eval(input("Enter coordinates for the p0 is x,y "))
        #随机点位置：x1, y1 = eval(input("Enter coordinates for the p1 is x,y "))
        #取每个gmm部分的（x，y）投影：x2, y2 = eval(input("Enter coordinates for the p2 is x,y "))
        a = y1-y0
        b = x0-x1
        c = (y0 - y1) * x0 + (x1 - x0) * y0  
    ...
    def callback_nav(self, data): #监听pointcloud,自己新建地图,并且保存对应的 odom.
        ...
        global gmm_map
        gmm_cnt=gmm_cnt+1
        print("gmmmap.mix_num",data.mix_num)
        gmm_map=data
    ...
    def gmm_nav(self,self_id):
        ...
        #发布coverage-time
        ratio_of_coverage = float(len(self.gmm_ground_all)/(self.W*self.L-24))
